...
echo
echo "-------------------------------------------------------------------------------"
echo "The word counts above are a size map for the analysis, not tokens. The harness-audit"
echo "skill turns them into an ESTIMATED cost (x~1.3), labeled as an estimate, and reports"
echo "everything above without any further input — one command, one report."
echo
echo "OPTIONAL — to swap the estimate for exact numbers, run these in a FRESH session"
echo "(after /clear, before working) and paste the output; the skill refines only the cost:"
echo "  /context   — real per-category token cost of your window (preload, tools, MCP,"
echo "               agents, memory, skill listing). Every line except conversation is the"
echo "               harness's fixed per-session cost; the conversation line is not the harness."
echo "  /doctor    — the platform's own health pass: unused skills/servers this session vs"
echo "               their cost, CLAUDE.md bloat, slow hooks, skill-listing budget overrun."
